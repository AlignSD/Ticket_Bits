...
contract('Marketplace', ([deployer, seller, buyer]) => {
    ...
    describe('tickets', async() => {
       let result, ticketCount
        before(async () => {
            result = await marketplace.createTicket('Crssd', web3.utils.toWei('1', 'Ether'), '3/20/21', 'San Diego', 'House Music', '1')
            console.log(result.args, "results.args");
        ...
        it('creates tickets', async () => {
            ...
            assert.equal(event.available, "1", 'is correct')


            //FAILURES: Product must have a name
             await marketplace.createTicket('', web3.utils.toWei('1', 'Ether'), '3/20/21', 'San Diego', 'House Music', { from: seller}).should.be.rejected;
            //FAILURES: Product must have a price
             await marketplace.createTicket('Crssd', 0, '3/20/21', 'San Diego', 'House Music', { from: seller}).should.be.rejected;
            //FAILURES: Product must have a date
             await marketplace.createTicket('Crssd', web3.utils.toWei('1', 'Ether'), '', 'San Diego', 'House Music', { from: seller}).should.be.rejected;
            //FAILURES: Product must have a location
             await marketplace.createTicket('Crssd', web3.utils.toWei('1', 'Ether'), '3/20/21', '', 'House Music', { from: seller}).should.be.rejected;
            //FAILURES: Product must have a description
             await marketplace.createTicket('Crssd', web3.utils.toWei('1', 'Ether'), '3/20/21', 'San Diego', '', { from: seller}).should.be.rejected;
            //FAILURES: Product must have a avialble tickets
             await marketplace.createTicket('Crssd', web3.utils.toWei('1', 'Ether'), '3/20/21', 'San Diego', 'House Music', "0", { from: seller}).should.be.rejected;
             
        })

        // it ('list tickets', async() => {
        //     const ticket = result.logs[0].args
        ...
        //     assert.equal(ticket.available, '1', 'is correct')
        ...
        it ('sells tickets', async() => {
            ...
            result = await marketplace.purchaseTicket({ from: buyer, value: web3.utils.toWei('1', 'Ether')})
            ...
            assert.equal(event.id.toNumber(), ticketCount.toNumber(), 'id is correct')
            assert.equal(event.name, 'Crssd', 'is correct')
            assert.equal(event.price, '1', 'is correct')
